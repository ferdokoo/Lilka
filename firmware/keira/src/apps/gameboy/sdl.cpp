...
unsigned int *gb_sdl_get_framebuffer(void)
{
...
unsigned int gb_sdl_get_buttons(void)
{
	...
	return (st.a.pressed*8) | (st.b.pressed*4) | (st.select.pressed*2) | (st.start.pressed*1)
}

unsigned int gb_sdl_get_directions(void)
{
    lilka::State st = lilka::controller.getState();
	return (st.up.pressed*8) | (st.right.pressed*4) | (st.down.pressed*2) | (st.left.pressed*1)
}
