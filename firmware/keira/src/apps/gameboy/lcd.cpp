...
unsigned char gb_lcd_get_stat(void)
{

	return (ly_int)<<6 | lcd_mode;
}

void gb_lcd_write_bg_palette(unsigned char n)
{
	bgpalette[0] = (n>>0)&3;
	bgpalette[1] = (n>>2)&3;
	bgpalette[2] = (n>>4)&3;
...
void gb_lcd_write_spr_palette1(unsigned char n)
{
	sprpalette1[0] = 0;
	sprpalette1[1] = (n>>2)&3;
	sprpalette1[2] = (n>>4)&3;
...
void gb_lcd_write_spr_palette2(unsigned char n)
{
	sprpalette2[0] = 0;
	sprpalette2[1] = (n>>2)&3;
	sprpalette2[2] = (n>>4)&3;
...
void gb_lcd_write_scroll_x(unsigned char n)
{
//	printf("x scroll changed to %02x\n", n);
	scroll_x = n;
}

void gb_lcd_write_scroll_y(unsigned char n)
{
	scroll_y = n;
}
...
void gb_lcd_write_stat(unsigned char c)
{
	ly_int = !!(c&0x40);
}

void gb_lcd_write_control(unsigned char c)
{
...
void gb_lcd_set_ly_compare(unsigned char c)
{
	lcd_ly_compare = c;
}

void gb_lcd_set_window_y(unsigned char n) {
	window_y = n;
}

void gb_lcd_set_window_x(unsigned char n) {
	window_x = n;
}
...
static void draw_bg_and_window(byte *b, int line)
{
	...
	for(x = 0; x < 160; x++)
	{
		...
		tile_num = gb_mem_get_raw(0x9800 + map_select*0x400 + map_offset);
		if(bg_tiledata_select)
			tile_addr = 0x8000 + tile_num*16;
		else
			tile_addr = 0x9000 + ((signed char)tile_num)*16;

		b1 = gb_mem_get_raw(tile_addr+(ym%8)*2);
		b2 = gb_mem_get_raw(tile_addr+(ym%8)*2+1);
		mask = 128>>(xm%8);
...
static void draw_sprites(byte *b, int line, int nsprites, struct sprite *s)
{
	...
	for(i = 0; i < nsprites; i++)
	{
		...
		b1 = gb_mem_get_raw(tile_addr);
		b2 = gb_mem_get_raw(tile_addr+1);
...
static void render_line(int line)
{
	...
	struct sprite s[10];
	byte *b = gb_sdl_get_framebuffer();

	for(i = 0; i<40; i++)
	{
		int y;

		y = gb_mem_get_raw(0xFE00 + (i*4)) - 16;
		if(line < y || line >= y + 8+(sprite_size*8))
			continue;

		s[c].y     = y;
		s[c].x     = gb_mem_get_raw(0xFE00 + (i*4) + 1)-8;
		s[c].tile  = gb_mem_get_raw(0xFE00 + (i*4) + 2);
		s[c].flags = gb_mem_get_raw(0xFE00 + (i*4) + 3);
		c++;
...
static void draw_stuff(void)
{
  byte *b = gb_sdl_get_framebuffer();
	...
	for(y = 0; y<8; y++)
	{
		...
		b1 = gb_mem_get_raw(tileaddr);
		b2 = gb_mem_get_raw(tileaddr+1);
   /*
...
int gb_lcd_cycle(void)
{
	...
	if(ly_int && lcd_line == lcd_ly_compare)
		gb_interrupt(INTR_LCDSTAT);

	if(prev_line == 143 && lcd_line == 144)
	{
		//draw_stuff();
		gb_interrupt(INTR_VBLANK);
		gb_sdl_frame();
	}
	prev_line = lcd_line;
	return 1;
