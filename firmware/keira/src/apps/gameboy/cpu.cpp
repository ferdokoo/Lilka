...
static void RLC(unsigned char reg)
{
	...
	switch(reg)
	{
		...
		break;
		case 6:	/* (HL) */
			t = gb_mem_get_byte(get_HL());
			old = !!(t&0x80);
			set_C(old);
			t = t<<1 | old;
			gb_mem_write_byte(get_HL(), t);
			set_Z(!t);
		break;
		case 7:	/* A */
			old = !!(c.A&0x80);
...
void gb_cpu_interrupt(unsigned short vector)
{
	halted = 0;

	c.SP -= 2;
	gb_mem_write_word(c.SP, c.PC);
	c.PC = vector;
	gb_interrupt_disable();
}
...
int gb_cpu_cycle(void)
{
	...
	if(gb_interrupt_flush())
	{
		halted = 0;
	}


	b = gb_mem_get_byte(c.PC);
...
//		is_debugged = 0;
	...
	switch(b)
	{
		...
		case 0x01:	/* LD BC, imm16 */
			s = gb_mem_get_word(c.PC+1);
			set_BC(s);
			c.PC += 3;
			c.cycles += 3;
		break;
		case 0x02:	/* LD (BC), A */
			gb_mem_write_byte(get_BC(), c.A);
			c.PC += 1;
			c.cycles += 2;
		break;
		case 0x03:	/* INC BC */
		...
		case 0x08:	/* LD (imm16), SP */
			gb_mem_write_word(gb_mem_get_word(c.PC+1), c.SP);
			c.PC += 3;
			c.cycles += 5;
		break;
		case 0x09:	/* ADD HL, BC */
		...
		break;
		case 0x18:	/* JR rel8 */
			c.PC += (signed char)gb_mem_get_byte(c.PC+1) + 2;
			c.cycles += 3;
		break;
		case 0x19:	/* ADD HL, DE */
			i = get_HL() + get_DE();
		...
		break;
		case 0x70:	/* LD (HL), B */
			gb_mem_write_byte(get_HL(), c.B);
			c.PC += 1;
			c.cycles += 2;
		break;
		case 0x71:	/* LD (HL), C */
			gb_mem_write_byte(get_HL(), c.C);
			c.PC += 1;
			c.cycles += 2;
		break;
		case 0x72:	/* LD (HL), D */
			gb_mem_write_byte(get_HL(), c.D);
			c.PC += 1;
			c.cycles += 2;
		break;
		case 0x73:	/* LD (HL), E */
			gb_mem_write_byte(get_HL(), c.E);
			c.PC += 1;
			c.cycles += 2;
		break;
		case 0x74:	/* LD (HL), H */
			gb_mem_write_byte(get_HL(), c.H);
			c.PC += 1;
			c.cycles += 2;
		break;
		case 0x75:	/* LD (HL), L */
			gb_mem_write_byte(get_HL(), c.L);
			c.PC += 1;
			c.cycles += 2;
		break;
		case 0x76:	/* HALT */
		...
		case 0xCB:	/* RLC/RRC/RL/RR/SLA/SRA/SWAP/SRL/BIT/RES/SET */
			decode_CB(gb_mem_get_byte(c.PC+1));
			c.PC += 2;
			c.cycles += 2;
		break;
		...
		case 0xE0:	/* LD (FF00 + imm8), A */
			t = gb_mem_get_byte(c.PC+1);
			gb_mem_write_byte(0xFF00 + t, c.A);
			c.PC += 2;
			c.cycles += 3;
		break;
		case 0xE1:	/* POP HL */
			i = gb_mem_get_word(c.SP);
			set_HL(i);
			c.SP += 2;
			c.PC += 1;
			c.cycles += 3;
		...
		case 0xE2:	/* LD (FF00 + C), A */
			s = 0xFF00 + c.C;
			gb_mem_write_byte(s, c.A);
			c.PC += 1;
			c.cycles += 2;
		break;
		case 0xE5:	/* PUSH HL */
			c.SP -= 2;
			gb_mem_write_word(c.SP, get_HL());
			c.PC += 1;
			c.cycles += 3;
		break;
		...
		case 0xF2:	/* LD A, (FF00 + c) */
			c.A = gb_mem_get_byte(0xFF00 + c.C);
			c.PC += 1;
			c.cycles += 2;
		break;
		case 0xF3:	/* DI */
